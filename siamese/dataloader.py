...
def get_transform(train):
    return RandomChoice(train_transforms()) if train else RandomChoice([None])
...
class CXRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, x):
        ...
        before_image = Image.open(os.path.join(self.root, filepath_before))
        after_image = Image.open(os.path.join(self.root, filepath_after))
        if self.transforms is not None:
            before_image, after_image = self.transforms([before_image, after_image])
...
def get_dataloader(base_root, csv_name, batch_size, seed=42):
    ...
    train_indices, val_indices = train_test_split(range(annotations_df.shape[0]), test_size=0.8, random_state=seed)
    ...
    trainloader =  torch.utils.data.DataLoader(tarin_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    validloader =  torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
