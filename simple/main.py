...
def main():
    ...
    model = Simple(args.num_classes, args.model_name, True)
    ...
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=10, factor=0.5, min_lr=1e-6, mode="max")
    ...
    for epoch in range(args.epochs): 
        train_loss, train_acc, train_f1 = train_one_epoch(model, trainloader, optimizer, scheduler, criterion, device)
        valid_loss, valid_acc, valid_f1 = validation(model, validloader, criterion, device)
