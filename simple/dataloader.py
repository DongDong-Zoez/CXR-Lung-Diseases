...
class CXRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, x):
        ...
        before_image = np.array(Image.open(os.path.join(self.root, filepath_before)).convert('L'))
        after_image = np.array(Image.open(os.path.join(self.root, filepath_after)).convert('L'))
        fusion_image = before_image + after_image
        image = np.stack([before_image, after_image, fusion_image], axis=-1)
...
def get_dataloader(base_root, csv_name, batch_size, seed=42):
    ...
    trainloader =  torch.utils.data.DataLoader(tarin_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
